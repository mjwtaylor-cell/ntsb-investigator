...
function main(): void {
  ...
  const summary = [
    ...
    `Burn rate: ${bundle.par.expectedBurnPerDay.toFixed(2)} inv-days/day (expected 5 groups)`,
    `Budget:    ${Math.ceil(bundle.par.investigatorDays * (bundle.truth.difficulty === 'senior' ? 1.1 : 1.5))} inv-days starting (${bundle.truth.difficulty === 'senior' ? 'par×1.1' : 'par×1.5'})`,
    `Par set:   ${bundle.par.evidenceSet.join(', ')} (Σ cost ${bundle.par.evidenceCostSum})`,
